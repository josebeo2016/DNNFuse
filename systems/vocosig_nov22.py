...
model_path = os.path.join(BASE_DIR, "scl_vocosig/pretrained/conf-5-linear-nov22.pth")
threshold = -2.4381
device = 'cuda' if torch.cuda.is_available() else 'cpu'

config = os.path.join(BASE_DIR,'scl_vocosig/configs/conf-5-linear.yaml')
# load config file
...
def parse_input(file_path):
    ...
    X_pad = pad(X, "zero",max_len=cut)
    x_inp = Tensor(X_pad)
    return x_inp.unsqueeze(0).to(device)

def vocosig_detect(wav):
...
def vocosig_detect_sig(y,sr=16000, **args):
    x_inp = parse_input_sig(y,fs=sr, **args)
    out = model(x_inp)
    per = nn.Softmax(dim=1)(out)
...
def vocosig_score_sig(y, sr=16000, sensitivity: float=1.0):
...
def vocosig_score_chunk(ys: list, sr: int = 16000, sensitivity: float=1.0):
    """
    Get a list of audio chunks and return a average of the prediction
    """
    res = []
    for y in ys:
        res.append(vocosig_score_sig(y, sr, sensitivity))
    return sum(res)/len(res)

def vocosig_detect_chunk(ys: list, sr=16000):
    res = []
    for y in ys:
        res.append(vocosig_detect_sig(y, sr))
    start = 0
